...
const rootReducer = combineReducers({ loading, form, user, hero, stew, noodle, curry, steak, soup, salad, baking, burger, pizza, cake, dessert, drink, posts, postWrite, post, postModify, follow, comment });

export function* rootSaga() {
  // yield는 비동기 작업을 처리할 때 작업 단위를 나누는 기준점 같은 것으로 보면 됩니다.
  // all 은 여러 개의 Saga를 동시에 실행시켜줍니다.
  yield all([userSaga(), heroSaga(), stewSaga(), noodleSaga(), currySaga(), steakSaga(), soupSaga(), saladSaga(), bakingSaga(), burgerSaga(), pizzaSaga(), cakeSaga(), drinkSaga(), dessertSaga(), postsSaga(), postWriteSaga(), postSaga(), postModifySaga(), followSaga(), commentSaga()]);
}
