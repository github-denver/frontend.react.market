...
export default createReducer(initialState, (builder) => {
  builder
    .addCase(POST_MODIFY_INITIAL, (state, action) => {
      console.group('[POST_MODIFY_INITIAL]: (state, action) => { .. }');
      state.data = null;
      state.error = null;
      console.groupEnd();
    })
    .addCase(POST_MODIFY_SUCCESS, (state, action) => {
      console.group('[POST_MODIFY_SUCCESS]: (state, action) => { .. }');
      state.data = action.payload;
      state.error = null;
      console.groupEnd();
    })
    .addCase(POST_MODIFY_FAILURE, (state, action) => {
      console.group('[POST_MODIFY_FAILURE]: (state, action) => { .. }');
      state.data = null;
      state.error = action.payload;
      console.groupEnd();
    });
  // .addDefaultCase((state, action) => {
  //   console.group('[POST_MODIFY_FAILURE]: addDefaultCase((state, action) => { .. })');
  //   console.groupEnd();
  // });
});
