...
import ArticleModifyComponent from "@/components/article/modify";
...
const ArticleModifyContainer = ({ attribute }) => {
  ...
  const { user, read, owner, subject, contents, thumbnail } = useSelector(
    ({ user, boardRead, boardModify }) => {
      const temp = {};
  ...
  const field = useCallback(
    (payload) => dispatch(formChangeField(payload)),
    [dispatch]
  );

  const upload = useCallback(
    (payload) => dispatch(formChangeThumbnail(payload)),
    [dispatch]
  );
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(
        "boardRead/modify 언 마운트 될 때 리덕스에서 데이터를 삭제합니다."
      );
