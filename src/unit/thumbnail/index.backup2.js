...
const ThumbnailUnit = ({ attribute }) => {
  const { className, label, timer, sticker, bookmark, title, author, options, bar, radius = true, space, src, href } = attribute || {}
  ...
  return (
    <StyledThumbnail className={className}>
      ...
      {title && <StyledTitleThumbnail to={href} $space={space} dangerouslySetInnerHTML={{ __html: thumbnailDescription }} />}
